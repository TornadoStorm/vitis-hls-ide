...
export default async (solution: SolutionInfo) => {
    ...
    const tclFilePath = path.join(tempDir, `csynth-${solution.project.name}-${solution.name}-${Date.now()}.tcl`);

    fs.writeFile(tclFilePath, tclContent, (err) => {
        if (err) {
            vscode.window.showErrorMessage(`Error writing tcl file for C synthesis: ${err.message}`);
            return;
        }
        ...
        const taskEndListener = vscode.tasks.onDidEndTaskProcess(async e => {
            if (e.execution.task.name === task.name) {
                ...
                fs.unlink(tclFilePath, (err) => {
                    if (err) {
                        vscode.window.showErrorMessage(`Error deleting tcl file for C synthesis: ${err.message}`);
                    }
                });
                ...
                if (e.exitCode !== 0) {
                    OutputConsole.instance.appendLine('C synthesis failed');
                    return;
                } else {
                    OutputConsole.instance.appendLine('C synthesis finished');
                }
            }
        });
    });
